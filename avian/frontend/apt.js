(function () {
  ...
  var SKETCH_VERSION = 'r15'; // local library rebuild; keep newer fork revision
  ...
  var IMG_VERSION = 'r14'; // keep newer fork-wide image revision
  ...
  function renderCollage(items, animate) {
    ...
    placed.forEach(function (r) {
      ...
      var titlePeriod = educatorScopeId()
        ? 'durant ' + educatorScopeLabel(effectiveEducatorScope)
        : windowLabel(currentHours, DATA.recent);
      btn.title = shownName + ' - ' + fmtN(titleN) + ' ' +
        (titleN === 1 ? 'detecció' : 'deteccions') + ' ' + titlePeriod;
  ...
  function windowLabel(h, windowData) {
    if (windowData && windowData.midnight_clamped) return 'des de mitjanit';
  ...
  function renderStatsLists() {
    ...
    if (firstSeenCap) firstSeenCap.textContent = scopedLabel
      ? 'primeres deteccions durant ' + scopedLabel
      : past
    ...
    document.getElementById('statsByPeriod').innerHTML = scopedLabel
      ? liRow('DETECCIONS', 'deteccions', fmtN(all_det))
        + liRow('OCELLS', 'espècies úniques', fmtN((((DATA.lifelist || {}).species) || []).length))
  ...
  function renderRhythm() {
    ...
    if (title && cap) {
      if (educatorScopeId()) {
        title.textContent = 'Ritme d’escolta';
        cap.textContent = 'deteccions durant ' + educatorScopeLabel(effectiveEducatorScope);
  ...
  function renderAtlas(animate) {
    ...
    var cardHtml = species.map(function (s) {
      ...
      var allLabel = educatorScopeId() ? educatorScopeLabel(effectiveEducatorScope) : 'total';
  ...
  document.addEventListener('visibilitychange', function () {
    if (document.hidden) {
      ...
      else if (!modalIsOpen()) runPeriodicRefresh(true);
      ...
      if (r.status === 200) {
        ...
        return r.json().catch(function () { return {}; }).then(function (body) {
          lockHint.textContent = body.recovery
            ? "La contrasenya d'administració no existeix o no és vàlida. Per SSH, executa sudo /usr/local/sbin/avian-admin-control password-reset."
            : 'Contrasenya incorrecta.';
          lockHint.classList.add('lock-err');
          passInput.focus();
        });
      } else if (r.status === 429) {
        lockHint.textContent = 'Massa intents. Torna-ho a provar d’aquí a una estona.';
  ...
  function menuItemMarkup(it) {
    ...
    var rawLabel = it && it.label || '';
    var label = escHtml(menuLabelsCa[String(rawLabel).toLowerCase()] || rawLabel);
  ...
  function themeRow() {
    ...
    return ''
      ...
      + '  <div class="seg settings-theme-seg" data-theme-seg role="group" aria-label="Tema"><i class="seg-pill" aria-hidden="true"></i>'
      + btn('auto', 'Segueix el tema del sistema', 'automàtic') + btn('light', 'Utilitza el tema clar', 'clar') + btn('dark', 'Utilitza el tema fosc', 'fosc') + '</div>'
  ...
  function labelsRow() {
    ...
    return ''
      ...
      + '  <button type="button" class="switch" role="switch" aria-label="Mostra els noms dels ocells"'
  ...
  function atlasAlwaysAllRow() {
    ...
    return ''
      + '<div class="menu-row">'
      + '  <div><span class="label">Mostra sempre tot l’atles</span><span class="hint">mostra tots els segells desbloquejats</span></div>'
      + '  <button type="button" class="switch" role="switch" aria-label="Mostra sempre tot l’atles"'
  ...
  function atlasClassicRow() {
    ...
    return ''
      + '<div class="menu-row">'
      + '  <div><span class="label">Targetes clàssiques de l’atles</span></div>'
      + '  <button type="button" class="switch" role="switch" aria-label="Targetes clàssiques de l’atles"'
  ...
  function saveSettings() {
    ...
    return adminFetch('./avian/api/config.php', {
      ...
      .catch(function (error) {
        ...
        restoreSubmittedSettings(submitted);
        setSaveState('error de xarxa', 'err');
      })
      .then(function () {
  ...
  function watchGenerate(btn, sci, stillThere, onDone, job) {
    ...
    adminJson('./avian/api/generate.php?action=status').then(function (s) {
      ...
      if (adminAuthCancelled(error)) {
        if (stillThere()) genBtnState(btn, 'desbloqueja el menú per comprovar el progrés', false);
        return;
      }
      finishActiveGenerate(job);
    ...
    adminFetch('./avian/api/generate.php?action=start', {
      ...
      .then(function (res) {
        ...
        if (!res.ok) {
          ...
          finishActiveGenerate(job);
          genBtnState(btn, why === 'no gemini key' ? 'afegeix una clau de Gemini a la configuració' : why, false);
  ...
  function populatePostcard(
    ...
    if (genBtn) {
      ...
      if (needsArt) genBtnState(genBtn,
        adminAccessState === 'unlocked' ? 'genera la imatge' : 'desbloqueja el menú per generar-la',
        false);
    ...
    Promise.all([
      ...
      .then(function (parts) {
        ...
        adminBody.innerHTML =
          ...
          + atlasAlwaysAllRow()
          + atlasClassicRow()
          + settingsText('SITE_NAME', 'Nom de l’estació', v.SITE_NAME || 'BirdNET-Pi', 60)
          ...
          + settingsToggle('RESET_AT_MIDNIGHT', 'Reinicia a mitjanit', 'manté les finestres temporals dins del dia actual', !!v.RESET_AT_MIDNIGHT)
          + '</section><section>'
          + stationRow(v)
        ...
        // Els noms dels ocells s'apliquen i es desen immediatament sense entrar
        // a la cua de configuració del Pi.
        wireLabelsPreference(adminBody);
        // Preferències només de l'Atles: s'apliquen sense modificar el selector
        // de temps compartit ni enviar cap configuració a l'estació.
  ...
  function adminSystemMarkup(j) {
    ...
    Object.keys(svc).forEach(function (name) {
      ...
      html += '<tr>'
        ...
        + '<td>' + (s.restartable
          ? '<button class="restart" data-unit="' + adminEsc(name) + '">reinicia</button>'
          : '<span class="muted">només estat</span>') + '</td>'
            ...
            return response.json().catch(function () { return {}; }).then(function (body) {
              if (!response.ok || !body.ok || !/^[a-f0-9]{48}$/.test(body.token || '')) {
                var error = new Error(response.status === 413
                  ...
                  : 'La descàrrega no està disponible.');
                error.status = response.status;
                throw error;
              }
