...
assert.match(apt, /settingsSlider\('OVERLAP',[\s\S]{0,180}settingsToggle\('RESET_AT_MIDNIGHT', 'Reinicia a mitjanit'/,
...
assert.equal(windowLabelContext.windowLabel(24, { midnight_clamped: true }), 'des de mitjanit',
  'a clamped window is labeled from station midnight');
assert.equal(windowLabelContext.windowLabel(24, { midnight_clamped: false }), 'avui',
  'an unclamped 24 hour window keeps its rolling label');
assert.equal(windowLabelContext.windowLabel(168, null), 'aquesta setmana',
...
assert.match(html, /apt\.js\?v=r235/, 'the polished behavior has a fresh cache key');
